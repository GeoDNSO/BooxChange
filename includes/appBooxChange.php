...
class appBooxChange
{
    ...
    public function valorarLibro($id, $valoracion, $idUsuario, $comentario){
        ...
        $bdBooxChange->valorarLibro($id, $valoracion, $idUsuario, $comentario);

    }

    public function valoracionesLibro($idLibro){
        $bdBooxChange = DAOValoracionLibro::getInstance();
        $valoracionesLibro = $bdBooxChange->valoracionesLibro($idLibro);
    ...
    public function subirOfertaLibro($libroQuerido, $libroOfertado)
    {
        ...
        $bdBooxChange = DAOUsuario::getInstance();
        $usuarioOfertado = $bdBooxChange->getUserById($libroOfertado->getIdUsuario());
        $usuarioQuerido = $bdBooxChange->getUserById($libroOfertado->getIdUsuario());

        //Notificamos al usuario del libro querido
        $bdBooxChange = DAONotificacion::getInstance();
        $bdBooxChange->notificarOferta($libroQuerido, $libroOfertado, $usuarioQuerido, $usuarioOfertado);
